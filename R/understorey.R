understorey <- function(realdata, trollsim, X = "X", Y = "Y", dbh = "dbh", sp_lab = "sp_lab", path)
{
...
# Matching individuals ----------------------------------------------------
  ...
  row_Xdat <- which(realdata[,Xdat] == trollsim[,Xsim])
  row_Xsim <- which(trollsim[,Xsim] == realdata[,Xdat])
  row_XYdat <- which(realdata[row_Xdat,Ydat]== trollsim[row_Xsim,Ysim])
  row_XYsim <- which(trollsim[row_Xsim,Ysim] == realdata[row_Xdat,Ydat])
  
  commons <- which(realdata[row_XYdat, sp_labdat] == trollsim[row_XYsim,sp_labsim])
...
# Replacement procedure ---------------------------------------------------
  ...
  for(i in 1:nrow(trollsim))
  {
    ...
    {
      if(realdat[row_xyi,sp_labdat] == trollsim[i,sp_labsim]){
    ...
    else
    {
      if(trollsim[i,dbhsim]<100) #Gotta get sure, because TROLLsims have dbhs very low (under 1)
      {
        ...
        realdata <- rbind(realdata,trollsim[i,])
      }
  ...
  journal <- file(file.path(path, 'understorey_journal.txt'), open = 'a')
